...
def upgrade() -> None:
    ...
    exportation_category_enum = postgresql.ENUM(
        'VINHO', 'ESPUMANTES', 'UVA', 'SUCO', 
        name='exportation_category'
    )
    exportation_category_enum.create(op.get_bind())
    
    # Add the category column (nullable first)
    op.add_column('exportation', sa.Column('category', exportation_category_enum, nullable=True))
    ...
    op.alter_column('exportation', 'quantity_kg',
               existing_type=sa.DOUBLE_PRECISION(precision=53),
               type_=sa.Integer(),
               existing_nullable=False)
    op.alter_column('exportation', 'value',
               existing_type=sa.DOUBLE_PRECISION(precision=53),
               type_=sa.Integer(),
               existing_nullable=False)
    ...
    op.create_index(op.f('ix_exportation_category'), 'exportation', ['category'], unique=False)
...
def downgrade() -> None:
    ...
    op.add_column('exportation', sa.Column('path', sa.VARCHAR(), autoincrement=False, nullable=False))
